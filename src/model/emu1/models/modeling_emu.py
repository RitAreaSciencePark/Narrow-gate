...
class Emu(nn.Module, PredictClassMixin):
    def __init__(
        ...
        vladapter_cfg = (
            VLadapterCfg(**vladapter_cfg)
            if isinstance(vladapter_cfg, dict)
            else vladapter_cfg
        )
        ...
        from src.model.emu1.models.modeling_llama import LLaMAForClsAndRegression
    ...
    def forward(
        ...
        if image_latent is None or image_features is None:
            image_features = self.visual.forward_features(pixel_values)
    ...
    def generate(
        ...
        **kwargs,
        ...
        if pixel_values is not None:
            image = pixel_values.to(dtype=torch.bfloat16)
            image_features = self.ln_visual(self.visual.forward_features(image))
            image_features = (
                self.cformer(image_features).squeeze().to(dtype=torch.bfloat16)
            )
        ...
        with torch.amp.autocast(
            device_type=self.args.device.type, dtype=torch.bfloat16
        ):
    ...
    def generate_image(
        ...
        if image is not None:
            ...
            prompt_image_embeds = prompt_image_embeds.view(
                -1, prompt_image_embeds.shape[-1]
            )
        ...
        for num_img_token in range(self.n_causal):
            ...
            cumsum_idx = torch.flip(
                torch.cumsum(torch.flip(image_idx, dims=[1]), dim=1), dims=[1]
            )
            if image is not None:
            ...
            if target_image_embeds is not None:
                target_idx = torch.logical_and(
                    image_idx,
                    torch.logical_and(cumsum_idx > 0, cumsum_idx <= num_img_token),
                )
            ...
            target_idx = torch.logical_and(
                image_idx,
                torch.logical_and(cumsum_idx > 0, cumsum_idx <= num_img_token + 1),
            )
