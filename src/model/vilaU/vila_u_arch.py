...
from src.model.vilaU.constants import (
...
from src.model.vilaU.configuration_vila_u import VILAUConfig
from src.model.vilaU.language_model.builder import build_llm_and_tokenizer
from src.model.vilaU.multimodal_encoder.builder import build_vision_tower
from src.model.vilaU.multimodal_encoder.rqvaesigliptransformer_encoder import RQVAESIGLIPTransformerVisionTower
from src.model.vilaU.multimodal_projector.builder import build_mm_projector
from src.model.vilaU.mutils import get_model_config
from src.model.vilaU.mm_utils import process_images
from src.model.vilaU.utils.media import extract_media
from src.model.vilaU.utils.tokenizer import infer_stop_tokens, tokenize_conversation
...
class VILAUMetaForCausalLM(ABC):
    ...
    def process_images(self, images):
        return process_images(images, self.vision_tower.image_processor, self.config)
    
    @torch.inference_mode()
    def generate_content(self, prompt: Union[str, List],images:Optional[list]=None, generation_config: Optional[GenerationConfig] = None) -> str:
        conversation = [{"from": "human", "value": prompt}]

        # media = extract_media(conversation, self.config)

        if images:
            images = process_images(images, self.vision_tower.image_processor, self.config).to(self.device, dtype=eval(self.config.model_dtype))
        else:
            images = None

        input_ids = tokenize_conversation(conversation, self.tokenizer, add_generation_prompt=True).to(self.device).unsqueeze(0)
