...
class Emu3Processor(ProcessorMixin):
    ...
    def __call__(
        ...
        *,
        ...
        chat_template : Optional[str] = "{image_prompt}{text_prompt}",
        **kwargs,
    ) -> BatchFeature:
        """
        ...
        if mode == 'G':
            ...
            if not isinstance(images, Sequence) and not isinstance(images, Image.Image):
                raise ValueError("Invalid input image. Please provide PIL.Image.Image or List[PIL.Image.Image].")

            if isinstance(images, Sequence) and not isinstance(images[0], Image.Image):
                raise ValueError("Invalid input image. Please provide PIL.Image.Image or List[PIL.Image.Image].")

            image_tokens = self.tokenize_image(images, padding_image=padding_image)
        ...
        for idx, text_prompt in enumerate(text):
            ...
            if mode == 'U':
                ...
                if chat_template:
                    prompt += chat_template.format(image_prompt=image_prompt, text_prompt=text_prompt)
                else:
                    prompt += text_prompt.format(IMG = image_prompt)
            else:
