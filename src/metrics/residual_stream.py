...
class ResidualStreamMetricComputer:
    def __init__(
        ...
        if residual_stream:
            ...
            self.num_layers = (
                len([el for el in residual_stream[self.modalities_name[0]].keys() if resid_key in el])
                if not resid_mid
                else int(len([el for el in residual_stream[self.modalities_name[0]].keys() if resid_key in el]) / 2)
            )
            self.num_examples = residual_stream[self.modalities_name[0]][f"{resid_key}_0"].shape[0]
        ...
        if head_out_resid:
            ...
            self.num_layers = max([
                                    ...
                                    for key in head_out_resid["image"].keys()
    ...
    def _assert_streams(self):
        ...
        if self.residual_stream:
            ...
            for modality in self.modalities_name:
                assert (
                    len([el for el in self.residual_stream[self.modalities_name[0]].keys() if self.resid_key in el]) == num_layer
            ...
            for modality in self.modalities_name:
                assert (
                    self.residual_stream[modality][f"{self.resid_key}_0"].shape[0]
                    == self.num_examples
    ...
    def _correlation_single_layer(self, stream: Dict[str, Dict[str, torch.Tensor]], resid_key: str, return_dist:bool = False) -> Dict[str,Tuple]:
        """
        ...
        for block in blocks:
            ...
            #Estimate the kde distribution
                ...
                # mean of similarity
                mean = block_data.abs().median().item()
                
                # get the absolute value of the cosine similarity
                #std of similarity
                std = block_data.std().item()
                
                # instead of std we consider 1st and 3rd quartile
                sorted_block_data = block_data.abs().sort()[0]
        ...
        if self.residual_stream:
            for layer in tqdm(range(self.num_layers), desc="Computing correlation residual out"):
                ...
                dist_single_layer = self._correlation_single_layer(
                    ...
                    resid_key=f"{self.resid_key}_{layer}",
