...
def pre_process_data(df, key):
    ...
    for index, row in df.iterrows():
        ...
        layers_str = str(layers_to_patch)
        ...
        if key == "logit_diff":
            logit_diff_patched = data["activations"]["logit_diff_in_patched"]
            logit_diff_clean = data["activations"]["logit_diff_in_clean"]

            # count the number of sign differences between the two tensors
            sign_diff = torch.sum(
                torch.sign(data["activations"]["logit_diff_in_patched"])
                != torch.sign(data["activations"]["logit_diff_in_clean"]),
                -1,
            )
            ...
            cat_meno_dog_patched = torch.softmax(
                data["activations"]["target_patched_logits"], -1
            )
            cat_meno_dog_clean = torch.softmax(
                data["activations"]["target_clean_logits"], -1
            )
            cat_clean = torch.softmax(data["activations"]["base_logits"], -1)
