...
def save_resid_hook(module, input, output,  cache, cache_key, token_index, img_token, avg:bool = False):
    """
    ...
    if avg:
        cache[f"avg_{cache_key}"] = torch.mean(b.data.detach().clone()[..., img_token, :], dim=-2, keepdims=True)
...
def avg_hook(module, input, output,  cache, cache_key, get_token_index):
    """
    It save the activations of the residual stream in the cache. It will save the activations in the cache (a global variable out the scope of the function)
    """
    if output is None:
    ...
    img_avg = torch.mean(b.data.detach().clone()[:, get_token_index(tokens=["all-image"]), :], dim=1,)
    text_avg = torch.mean(b.data.detach().clone()[:, get_token_index(tokens=["all-text"]), :], dim=1)
    all_avg = torch.mean(b.data.detach().clone()[:, :, :], dim=1)
    eols_avg = torch.mean(b.data.detach().clone()[:, get_token_index(tokens=["end-lines"]), :], dim=1)
    special_avg = torch.mean(b.data.detach().clone()[:, [0,1,2,32], :], dim=1,)

    cache[f"avg_{cache_key}"] = torch.cat(
        [img_avg.unsqueeze(1),
         text_avg.unsqueeze(1),
         all_avg.unsqueeze(1),
         special_avg.unsqueeze(1),
         eols_avg.unsqueeze(1)], dim=1
    )
...
# @profile
def ablate_resid_massive_act_hook(module, input, output, ablation_queries: pd.DataFrame):
    """
    Hook function to ablate massive activations in tokens specified in the ablation_queries dataframe.
    It will set to 0 the entries in the residual stream the are higher than the threshold*mean of the entries
    """
    if output is None:
        b = input[0]
    else:
        b = output[0]
    batch_size, seq_len, d_model = b.shape
    values = b.clone().data
    abs_values = torch.abs(values)
    device = values.device
    # Get the positions of the tokens to ablate
    positions = torch.tensor(ablation_queries["elem-to-ablate"].item()).squeeze(0).to(device)
    #assert len(positions.shape) == 1 or len(positions.shape) == 0
    # Used during generation
    if seq_len < torch.max(positions): # the ablation queries contains positions not yet generated
        return (b,)
    # Get the threshold
    threshold = 50
    # Get the mean of the values
    mean = torch.mean(abs_values[:,positions], dim=-1).squeeze(0)

    # Get the values to ablate
    values_to_ablate = abs_values[:,positions] > threshold * mean
    # Use advanced indexing to set the specified slices to zero
    values[:,positions,:][values_to_ablate] = 0
    b.copy_(values)
    b = (b,)

    if b[0].shape != (batch_size, seq_len, d_model):
        raise ValueError(f"Expected tensor shape ({batch_size}, {seq_len}, {d_model}), got {b[0].shape}")
    if b[0].device != device:
        raise ValueError(f"Expected tensor device {device}, got {b[0].device}")
    del values
    del abs_values
    return b
...
def ablate_attn_mat_hook(module, input, output, ablation_queries: pd.DataFrame):
    ...
    matrix. It will set to 0 the value vector of the
    tokens to ablate
    """
...
# def ablate_tokens_hook_flash_attn(module,
#                                   input,
#                                   output,
...
#     """
#     Hook function to ablate the tokens in the attention
#     mask. It set the mask that it is going to be passed
#     to flash attention
#     """
#     attn_mask, batch_size, num_heads, seq_len_q, seq_len_k, device = output
...
#     # Create boolean masks for queries and keys
#     q_mask = torch.zeros(seq_len_q, dtype=torch.bool, device=device)
#     q_mask[q_positions] = True  # Set positions to True

#     k_mask = torch.zeros(seq_len_k, dtype=torch.bool, device=device)
#     k_mask[k_positions] = True  # Set positions to TrueW

#     # Create a 2D mask using outer product
#     new_mask = torch.outer(q_mask, k_mask)  # Shape: (seq_len_q, seq_len_k)

#     # Expand mask to match the dimensions of the attention matrix
#     # Shape after expand: (batch_size, num_heads, seq_len_q, seq_len_k)
#     new_mask = new_mask.unsqueeze(0).unsqueeze(0).expand(batch_size, num_heads, -1, -1)
#     if attn_mask is not None:
#         attn_mask = attn_mask & ~new_mask
#     else:
#         attn_mask = ~new_mask

#     return attn_mask, batch_size, num_heads, seq_len_q, seq_len_k, device
...
def avg_attention_pattern_head(
    ...
    for head in range(num_heads):
        ...
        if key not in attn_pattern_current_avg:
            ...
            # move to cpu
            attn_pattern_current_avg[key] = attn_pattern_current_avg[key].cpu()
        else:
            attn_pattern_current_avg[key] += (
                 attn_pattern[:, head].cpu() - attn_pattern_current_avg[key]
            ) / (batch_idx+1)
