...
if __name__ == '__main__':
    ...
    for i in range(n_iterations):
        ...
        with torch.no_grad():
            for p_idx in tqdm.tqdm(idxs):
                ...
                elif args.mode == 'zero-shot' and args.model == 'pixtral':
                    prompt = f'[INST]{start_sentence}{image_string}{question}"\nAnswer the question using a single word or phrase."[/INST]'
                
                elif args.mode == 'prompt' and args.model == 'pixtral':
                    prompt = f'[INST]{start_sentence}{image_string}{question}{PROMT_PIXTRAL}[/INST]'

                elif args.mode == 'zero-shot' and args.model == 'emu3':
                    prompt = f'"{image_string}USER:{question}"\nAnswer the question using a single word or phrase."\nASSISTANT:"'

                elif args.mode == 'prompt' and args.model == 'emu3':
                    prompt = f'"{image_string}USER:{question}{PROMT_PIXTRAL}\nASSISTANT:"'
                ...
                if "chameleon" in args.model:
                    ...
                    inputs = processor(text=prompt, images=[img], return_tensors="pt", mode="U", chat_template=None).to(model.device())
                    inputs = {
        ...
        with open(f'{args.outdir}/{args.model}_{args.nsample}_{args.mode}_{args.window}.json', 'w') as f:
             json.dump(results, f, ensure_ascii=False)
        
        if args.model == 'pixtral' and args.mode == 'prompt':
            for i in range(len(results)):
                results[i]['answer'] = results[i]['answer'][results[i]['answer'].find('You must answer.')+16:]
        elif args.model == 'pixtral' and args.mode == 'zero-shot':
            for i in range(len(results)):
        ...
        elif args.model == 'emu3' and args.mode == 'prompt':
            for i in range(len(results)):
                results[i]['answer'] = results[i]['answer'][results[i]['answer'].find("ASSISTANT:")+10:]
        elif args.model == 'emu3' and args.mode == 'zero-shot':
            for i in range(len(results)):
                results[i]['answer'] = results[i]['answer'][results[i]['answer'].find("ASSISTANT:")+10:]
    ...
    print(f"Pixtral: {ev['pixtral']} +/- {ev['pixtral_std']}")
    
    with open(f'{args.outdir}/vqa_{args.model}_{args.nsample}_{args.mode}_{args.ablation}_eval.txt', 'w') as f:
